...
|--|--|--|--|--|--|--|--|
| s39 | 1022 | 339.2 | 3.7971 | 0.2437 | 5.4e-58 | 89.3 | 2P-ambiguous |
| s58 | 4607 | 307.0 | 3.7981 | 0.1051 | 2.0e-106 | 80.8 | star-cleaned:1,2P-ambiguous |

- Pipeline refine said: **1P** (folded amp_fourier 0.14); flags: clean
- DIA (de-comb): survived (verdict from a dedicated pass recorded in catalog/evidence.csv, not in this lot's refine output)
- Coverage: **1 sector(s)**, best sector covers **40.42 cycles** of the adopted period. Measured periodogram peak width 1% of the period, i.e. about **+/- 0.01863 h** (1 sigma); the decimal places in the adopted value are not a precision claim.
...
- doubling: by convention (one prominent maximum in the fold at P1 = 3.7985 h, stable in 100 per cent of block bootstraps, METHODOLOGY 3b-bis); the doubling is NOT established: the calibrated block-bootstrap test is silent (p_emp 0.049 and 0.125 on 20,000 null realisations, against a 0.001 bar), the two minima differ by 0.2 sigma and the folded amplitude at P1 is 0.122 mag, below the 0.40 cut, so the convention rests on the topology alone; both aliases 3.7985 h and 7.597 h are to be considered
...
![phase-fold at the adopted period](../plots/21691.png)
<!-- AUTO:END -->
...
2. **One global trend, not one per sector.** A single cubic plus two harmonics over
   the whole 307 h baseline, with the period search capped at
   baseline/8, that is at least eight rotations inside the data.
   Photometric period P1 = 3.7985 h, chi-squared gain 10.6 per cent.
...
4. **Independent subsets.** The two time halves return 7.581 and 3.799 h,
...
**One detecting sector (s58).** The coherence test is therefore between
...
| quantity at P1 = 3.7985 h | value |
|--|--|
| calibrated p, 1 d blocks | 0.04855 |
| calibrated p, 2 d blocks | 0.12514 |
| unequal minima, z above the null mean | +0.21 |
| prominent maxima in the fold (stability) | 1 (100 per cent) |
| folded amplitude at P1 | 0.122 mag |

**Adopted by convention, and the doubling is not established.** The fold at
3.7985 h has a single prominent maximum, stable in 100 per cent of the
...
the rotation period would be 3.7985 h; **both aliases are to be considered.**
...
## Verdict
CANDIDATE 2P / 7.597 h. The factor of two is adopted by convention and has no per-object evidence; the alias 3.7985 h is to be considered alongside it.
