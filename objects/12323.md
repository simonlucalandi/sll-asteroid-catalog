...
**Adopted:** 80.4884 h, 2P, CANDIDATE
...
|--|--|--|--|--|--|--|--|
| s20 | 812 | 630.5 | 40.2088 | 0.692 | 8.5e-203 | 15.7 | 2P-ambiguous |
...
- Coverage: **1 sector(s)**, best sector covers **7.83 cycles** of the adopted period. Measured periodogram peak width 5% of the period, i.e. about **+/- 0.9153 h** (1 sigma); the decimal places in the adopted value are not a precision claim.
...
- doubling: by convention (one prominent maximum in the fold at P1 = 40.2442 h, stable in 100 per cent of block bootstraps, and folded amplitude 0.646 mag above the 0.40 cut, METHODOLOGY 3b-bis); the doubling is NOT established: the calibrated block-bootstrap test is silent (p_emp 0.964 and 0.971 on 20,000 null realisations) and the two minima differ by -0.9 sigma; both aliases 40.2442 h and 80.4884 h are to be considered
- the photometric period lies 2.1 per cent from the 328.8/8 h momentum-dump tooth, where survival of the de-comb does not certify by construction: this is one of the two most fragile entries of the 2026-09 batch
...
![phase-fold at the adopted period](../plots/12323.png)
<!-- AUTO:END -->
...
2. **One global trend, not one per sector.** A single cubic plus two harmonics over
   the whole 630 h baseline, with the period search capped at
   baseline/8, that is at least eight rotations inside the data.
   Photometric period P1 = 40.2442 h, chi-squared gain 70.2 per cent.
...
4. **Independent subsets.** The two time halves return 40.244 and 39.821 h,
...
5. **De-comb.** TESS orbits in 328.8 h, so any periodogram has excess power at the
   teeth 328.8/n. Verdict for this object: **survived** (power change +1%, model
   R2 = 0.03).
   The photometric period sits **2.13% from the 41.1 h tooth (n = 8)**,
...
**One detecting sector (s20).** The coherence test is therefore between
...
| quantity at P1 = 40.2442 h | value |
|--|--|
| calibrated p, 1 d blocks | 0.96415 |
| calibrated p, 2 d blocks | 0.97130 |
| unequal minima, z above the null mean | -0.92 |
| prominent maxima in the fold (stability) | 1 (100 per cent) |
| folded amplitude at P1 | 0.646 mag |

**Adopted by convention, and the doubling is not established.** The fold at
40.2442 h has a single prominent maximum, stable in 100 per cent of the
...
the rotation period would be 40.2442 h; **both aliases are to be considered.**
...
## Verdict
CANDIDATE 2P / 80.4884 h. The factor of two is adopted by convention and has no per-object evidence; the alias 40.2442 h is to be considered alongside it.
