---
number: 1285
family: main belt (survey)
P_rot_h: 15.24
shape: 1P
status: CONFIRMED
quality_U: 2
updated: 2026-07-30
---

# (1285)

**Adopted:** 15.24 h, 1P, CONFIRMED

<!-- AUTO:START (regenerated from pipeline outputs; do not hand-edit this block) -->
## Evidence (auto)

Detected in 2 sector(s):

| sector | N | baseline (h) | P_phot (h) | power | FAP | cycles | flags |
|--|--|--|--|--|--|--|--|
| s42 | 1870 | 558.8 | 15.2386 | 0.2692 | 6.5e-123 | 36.7 | 2P-ambiguous |
| s91 | 5343 | 528.4 | 7.6206 | 0.3697 | 0.0e+00 | 69.3 | star-cleaned:62,2P-ambiguous |

- Pipeline refine said: **1P** (folded amp_fourier 0.077); flags: sick-dips-excised:s42(2),s91(31);harmonic-only-agreement:s91(pw=0.00);period-spread:67%
- DIA (de-comb): survived_v2(ret=0.980[0.97-0.99],s91@7.620h)
- Coverage: **2 sector(s)**, best sector covers **36.67 cycles** of the adopted period. Measured periodogram peak width 2% of the period, i.e. about **+/- 0.1427 h** (1 sigma); the decimal places in the adopted value are not a precision claim.
- Factor of two: no doubling was applied.
- Gates: FAP<1e-3 and power>=0.10 per detecting sector; >=2 sectors agree (harmonic-aware).
- Adopted shape: **1P** (folded-amplitude rule; pipeline agrees).

### Why this row reads the way it does

- RE-PROMOTED 2026-07-18 (user review + full DIA): 2-sector real. S42 15.24h FAP 1e-123 (0 contam); S91 contaminated (mag-7 star-crossing + faint dips) but census-cleaned it detects 7.62h=P/2 FAP~0 over 69 cyc. SysRem decomb/DIA: S42 15.24h survives (dPW -2%), S91 7.62h survives (dPW +13%) -> astrophysical not systematics. Folded amp 0.09(S42)/0.24(S91)<0.40 -> 1P. Disagrees with LCDB 20.3h. QUEUED ZTF on Fink recovery.

![phase-fold at the adopted period](../plots/1285.png)
<!-- AUTO:END -->

## Doubt
S91 flagged as contaminated (mag-7 star-crossing spike + faint dips to -2.5 mag); the DIA had never run on it (fell through the old comb/slow-only trigger and the range>3 mag sector-skip).
## Evidence
(a) S91 contamination is real but localized; the census bright-clip removes the 62 bad points and S91 still detects 7.62 h = P/2 (FAP ~0 over 69 cycles) -- a 1-2 day star-crossing cannot fake a 69-cycle coherent period. (b) S42 clean: 15.24 h, FAP 1e-123, 0 contamination. (c) SysRem DIA: S42 survives at 15.24 h (drop -2%), S91 survives at 7.62 h (drop +13%) -> astrophysical. (d) Folded amp 0.09 (S42) / 0.24 (S91), both < 0.40 -> 1P.
## Verdict
1P / 15.24 h, confirmed, 2-sector (S42 fundamental + S91 harmonic). Disagrees with the published LCDB 20.3 h (a genuine literature disagreement). This case motivated the contamination-triggered DIA rule.
