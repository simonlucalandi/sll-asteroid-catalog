...
**Adopted:** 55.355 h, ambiguous, CANDIDATE
...
|--|--|--|--|--|--|--|--|
| s65 | 7367 | 625.1 | 27.5701 | 0.3748 | 0.0e+00 | 11.3 | star-cleaned:24 |
...
- Coverage: **1 sector(s)**, best sector covers **11.29 cycles** of the adopted period. Measured periodogram peak width 9% of the period, i.e. about **+/- 2.037 h** (1 sigma); the decimal places in the adopted value are not a precision claim.
...
- doubling: ambiguous, P1 = 55.355 h is published and 2 x P1 = 110.710 h remains equally possible; the fold at P1 shows 2 prominent maxima stable in 100 per cent of the block bootstraps, so the topological form of the doubling convention says P1 is already the rotation period, while the folded amplitude 0.853 mag sits above the 0.40 cut: the two forms of the convention conflict and topology wins (specificity 1.000 against 0.871 on 174 external labels, METHODOLOGY 3b-bis); the calibrated test is silent (p_emp 0.243 and 0.263)
- the photometric period lies 1.0 per cent from the 328.8/6 h momentum-dump tooth, where survival of the de-comb does not certify by construction; the dedicated re-check puts the instrumental model power at the candidate period at 0.006 and preserves the amplitude to 6 per cent
...
![phase-fold at the adopted period](../plots/45880.png)
<!-- AUTO:END -->
...
2. **One global trend, not one per sector.** A single cubic plus two harmonics over
   the whole 625 h baseline, with the period search capped at
   baseline/8, that is at least eight rotations inside the data.
   Photometric period P1 = 55.3550 h, chi-squared gain 39.8 per cent.
...
4. **Independent subsets.** The two time halves return 54.55 and 55.355 h,
...
5. **De-comb.** TESS orbits in 328.8 h, so any periodogram has excess power at the
   teeth 328.8/n. Verdict for this object: **inconclusive** (power change +15%, model
   R2 = 0.01).
   The photometric period sits **1.00% from the 54.8 h tooth (n = 6)**,
   that is on a tooth. Survival of the de-comb there does not certify: 61 per
   cent of pure comb detections survive the projection by construction. These
   are the two most fragile entries of the batch and are labelled as such.



## What the coverage really is
**One detecting sector (s65).** The coherence test is therefore between
...
| quantity at P1 = 55.3550 h | value |
|--|--|
| calibrated p, 1 d blocks | 0.24284 |
| calibrated p, 2 d blocks | 0.26274 |
| unequal minima, z above the null mean | +2.05 |
| prominent maxima in the fold (stability) | 2 (100 per cent) |
| folded amplitude at P1 | 0.853 mag |

**Ambiguous, and published as such.** The photometric period 55.3550 h is the
published value and **2 x P1 = 110.7100 h remains exactly as possible**: nothing
...
This object is the one case of the fourteen where the two forms of the
convention **conflict**: the folded amplitude 0.853 mag is above the 0.40 cut,
which would force the doubling, while the fold shows two maxima stable in 100
per cent of the bootstraps, which says the photometric period is already the
rotation period. Topology wins, because on the external labels it has
specificity 1.000 against 0.871. That resolution was **not pre-registered** and
is declared here rather than presented as a rule.
...
## Verdict
CANDIDATE ambiguous / 55.355 h. The alias 2 x P1 = 110.7100 h is equally possible and neither is preferred.
